...
export default function EditTransactionScreen() {
  ...
  const form = useForm({
    defaultValues: {
      ...
      categoryId: transaction?.category_id ?? (null as number | null),
      sourceId: transaction?.source_id ?? (null as number | null),
      date: transaction?.date ?? format(new Date(), DATE_TIME_FORMAT),
  ...
  // Sync activeType when transaction loads
  if (transaction && activeType !== transaction.type) {
  ...
  return (
    <View className="flex-1 bg-background">
      ...
      >
        ...
        >
          {(field) => {
            const currentDate = parseDate(field.state.value);
            return (
              <View className="mb-5">
                <Text className="mb-1.5 text-sm font-medium text-muted-foreground">
                  Date & Time
