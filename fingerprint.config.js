 ...
 * config plugins (including custom ones, e.g. `plugins/firebase-ios-fix.js`'s
 * `withDangerousMod` Podfile edit), and app.json native fields. OTA updates
 * only reach builds whose fingerprint matches, so the runtime changes exactly
 * when (and only when) a new native build is required.
 *
