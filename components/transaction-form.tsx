...
export function TransactionForm({
  ...
  hideTags?: boolean;
  /**
   * True when editing an existing transaction (vs adding a new one). Gates
   * the active-scope auto-tag effect so editing a tag-less past transaction
   * doesn't silently pin it to the currently-active scope on save.
   */
  isEditing?: boolean;
}) {
  ...
  // Auto-tag with the currently active scope on the ADD flow only. Without
  // this gate, opening a tag-less past transaction in edit mode while a
  // scope is active would silently pin that transaction to the scope on save.
  // biome-ignore lint/correctness/useExhaustiveDependencies: run once on tag resolve
  useEffect(() => {
    if (isEditing) return;
    if (!activeTag) return;
    const current = form.getFieldValue("tagIds") ?? [];
    if (current.length > 0) return;
    form.setFieldValue("tagIds", [activeTag.id]);
  }, [activeTag?.id, isEditing]);
                        ...
                        {(amountField) => (
                          <View className="mt-3">
                            ...
                            <Text className="mt-1 text-xs text-muted-foreground">
                              Defaults to the full amount. Edit if only part
                              will be reimbursed.
                            </Text>
                          </View>
                        )}
