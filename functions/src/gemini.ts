...
async function callGemini<T>(
  ...
  const first = await callGeminiOnce<T>(apiKey, userContent, schema);
  if (first.error && TRANSIENT_ERRORS.includes(first.error)) {
    return callGeminiOnce<T>(apiKey, userContent, schema);
  }
  return first;
}
...
export async function parseWithGemini(
  ...
  >(apiKey, userContent, buildResponseSchema(uniqueNames));
