...
export function parseAxisBankEmail(body: string): ParsedTransaction | null {
	...
	const dateMatch = body.match(/Date & Time:\s*(\d{2}-\d{2}-\d{2})(?:\s+(\d{2}:\d{2}:\d{2}))?/i);
	...
	const parsed = parse(`${rawDate} ${rawTime}`, "dd-MM-yy HH:mm:ss", new Date());
	const date = format(parsed, "yyyy-MM-dd HH:mm");
	console.log("[Parser] Axis date:", rawDate, rawTime, "→", date);
...
export function parseHdfcEmail(body: string): ParsedTransaction | null {
	const match = body.match(/Rs\.([\d,]+\.?\d*) is debited from your HDFC Bank [\w\s]+ ending (\d+) towards (.+?) on (\d{2} \w+, \d{4})(?: at (\d{2}:\d{2}:\d{2}))?/i);
	...
	const parsed = parse(`${rawDate} ${rawTime}`, "dd MMM, yyyy HH:mm:ss", new Date());
	const date = format(parsed, "yyyy-MM-dd HH:mm");
	console.log("[Parser] HDFC date:", rawDate, rawTime, "→", date);
...
export function parseEmail(from: string, body: string): ParsedTransaction | null {
